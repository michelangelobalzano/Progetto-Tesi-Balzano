...
def objective(trial, model_params, device, run_name):
    ...
    with open('sessions\\classification_optimization' + run_name + '.csv', mode='a', newline='') as file:
        ...
        writer.writerow([val_accuracy, batch_size, d_model, dim_feedforward, dropout, num_heads, num_layers, pe_type])
...
def main(config):
    ...
    with open('sessions\\classification_optimization' + run_name + '.csv', mode='w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(['accuracy', 'batch_size', 'd_model', 'dim_feedforward', 'dropout', 'num_heads', 'num_layers', 'pe_type'])
